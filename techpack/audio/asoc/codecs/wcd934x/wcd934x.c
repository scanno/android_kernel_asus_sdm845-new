				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (!strcmp(w->name, "RX INT1_2 MUX")) {
				micb_val = snd_soc_read(codec, WCD934X_ANA_MICB2);
				if(micb_val==0x10){//normal mode 0x90:playback 0x50:incall 0x10:normal
					snd_soc_update_bits(codec, WCD934X_ANA_MICB2, 0xC0, 0x80);
					dev_dbg(codec->dev, "%s enable micb2 micb_val:0x%x\n", __func__, micb_val);//Micbais connected to supply
				}
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (!strcmp(w->name, "RX INT1_2 MUX")){
			micb_val = snd_soc_read(codec, WCD934X_ANA_MICB2);
			if(micb_val==0x90){//if is playback can reset
					snd_soc_update_bits(codec, WCD934X_ANA_MICB2, 0xC0, 0x00);
					dev_dbg(codec->dev, "%s disable micb2 micb_val:0x%x\n", __func__, micb_val);
				}
		} //disable micb2
