...
struct qusb_phy {
	...
	struct dentry		*root;
	u8			tune[5];
	u8			bias2;	//ASUS BSP ADD BIAS2 TUNE+++
...
static int qusb_phy_init(struct usb_phy *phy)
{
	...
	if (qphy->qusb_phy_init_seq)
		qusb_phy_write_seq(qphy->base, qphy->qusb_phy_init_seq,
				qphy->init_seq_len, 0);
	//ASUS BSP use asus EE's value +++,bit 4->6 should get value from qcom efuse reg
	if (qphy->efuse_reg) {
		if (!qphy->tune_val)
	...
	//ASUS BSP ADD BIAS2 TUNE+++
	if (qphy->bias2)
	{
		printk("ASUS adjust BIAS2 %d \n",qphy->bias2);
		writel_relaxed(qphy->bias2,qphy->base + qphy->phy_reg[BIAS_CTRL_2]);
	}
	//ASUS BSP ADD BIAS2 TUNE---
...
static int qusb_phy_create_debugfs(struct qusb_phy *qphy)
{
	...
	//ASUS BSP ADD BIAS2 TUNE+++
	snprintf(name, sizeof(name), "bias2");
	file = debugfs_create_x8(name, 0644, qphy->root,
					&qphy->bias2);
	if (IS_ERR_OR_NULL(file)) {
		dev_err(qphy->phy.dev,
