...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_RESULT_TOUCH_PATH, file_name);
    if (NULL == pfile) {
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
		if((LcdReTryTestResult == false) || (CbTestResult == false) || (cbReTryTestResult == false)
		|| (RawDataTotalTestResult == false) || (LCDTotalTestResult == false) || (shortTestResult==false)
		|| (openTestResult == false)){
			FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
			TpSelfTestFlag = false;
			CbTestResult = true;
    ...
    fts_test_save_test_data("TP_Raw_data.txt", test_data.testresult, test_data.testresult_len);
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_test_show(struct file *filp, char __user *buff, size_t len, loff_t *off)
{
    //<ASUS-BSP> show pass for factory test tool +++++
    char messages[256];
    ...
    if(TpSelfTestFlag)
        sprintf(messages, "PASS\n");
    else
        sprintf(messages, "FAIL\n");
...
* Return: char count
***********************************************************************/
static ssize_t fts_test_store(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    char fwname[128] = {0};
    ...
    if (copy_from_user(fwname, buff, len))
        return -EFAULT;
...
*/
static struct file_operations fts_test_proc_ops = {
...
{
    int ret = 0;
    struct proc_dir_entry *fts_test_proc_file;

    FTS_TEST_FUNC_ENTER();

    printk("[FTS][TOUCH] %s\n",__func__);

    fts_test_proc_file = proc_create("driver/fts_test", 0666, NULL, &fts_test_proc_ops);
    if (fts_test_proc_file == NULL){
        printk("[FTS][TOUCH] %s fts_test create failed\n",__func__);
        ret = -1;
    }else
		ret = 0;
