...
*****************************************************************************/
static bool fts_suspend_running = false;
static void fts_release_all_finger(void);
static int fts_ts_suspend(struct device *dev);
static int fts_ts_resume(struct device *dev);
//<ASUS-BSP> add touch test node ++++++
#define TOUCH_FUNCTION_PROC_FILE "driver/touch_function"
...
static ssize_t focal_touch_status_store(struct file *filp, const char __user *buff, size_t len, loff_t *off);

static struct file_operations touch_function_proc_ops = {
    .write = focal_touch_function_store,
    .read = focal_touch_function_show,
};

static struct file_operations touch_status_proc_ops = {
	.write = focal_touch_status_store,
	.read = focal_touch_status_show,
};
...
static void create_focal_touch_proc_file(void)
{
	struct proc_dir_entry *focal_touch_proc_file;

    printk("[FTS][TOUCH] create_focal_touch_proc_file\n");

    focal_touch_proc_file = proc_create(TOUCH_FUNCTION_PROC_FILE, 0666, NULL, &touch_function_proc_ops);
    if (focal_touch_proc_file == NULL)
    printk("[FTS][TOUCH]create_focal_touch_proc_file touch_function failed\n");

    focal_touch_proc_file = proc_create(TOUCH_STATUS_PROC_FILE, 0666, NULL, &touch_status_proc_ops);
    if (focal_touch_proc_file == NULL)
    printk("[FTS][TOUCH]create_focal_touch_proc_file touch_status failed\n");
}
...
static ssize_t focal_touch_function_store(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    char messages[256];
    memset(messages, 0, sizeof(messages));
    printk("[FTS][TOUCH] focal_touch_function_store\n");
...
static ssize_t focal_touch_status_show(struct file *filp, char __user *buff, size_t len, loff_t *off)
{
    ...
    retval = fts_i2c_read_reg(fts_data->client,FTS_REG_CHIP_ID,&status);
    if (retval < 0) {
        printk("[FTS][TOUCH]:%s: Failed to read device status, error = %d\n",
                __func__, retval);
       sprintf(messages, "0\n");
	}
...
static ssize_t focal_touch_status_store(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    printk("[FTS][TOUCH]attempted to write to read-only attribute touch_status");
    return -EPERM;
}
...
{
	struct fts_ts_data *fts_data =
			container_of(work, struct fts_ts_data,
			touch_suspend_work);

	fts_ts_suspend(&fts_data->client->dev);
	dev_info(&fts_data->client->dev,  "[FTS] touch suspend over!\n");
}
...
static void fts_resume_work(struct work_struct *work)
{
	struct fts_ts_data *fts_data =
			container_of(work, struct fts_ts_data,
			touch_resume_work);
	dev_info(&fts_data->client->dev,  "[FTS] touch resume start!\n");
                                ...
                                unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == MSM_DRM_EVENT_BLANK &&
        ...
        if (*blank == MSM_DRM_BLANK_UNBLANK){
            //fts_ts_resume(&fts_data->client->dev);
            FTS_INFO("receives BLANK:UNBLANK, run resume");
            fts_suspend_running = false;
            queue_work(fts_data->tp_suspend_resume_wk,&fts_data->touch_resume_work);
		}
        else if (*blank == MSM_DRM_BLANK_POWERDOWN){
            //fts_ts_suspend(&fts_data->client->dev);
            FTS_INFO("receives BLANK:POWERDOWN");
            //queue_work(fts_data->tp_suspend_resume_wk,&fts_data->touch_suspend_work);
		}
    }
    if (evdata && evdata->data && event == MSM_DRM_EARLY_EVENT_BLANK &&
        ...
        if (*blank == MSM_DRM_BLANK_UNBLANK){
            //fts_ts_resume(&fts_data->client->dev);
            FTS_INFO("receives EARLY_BLANK:UNBLANK");
            //queue_work(fts_data->tp_suspend_resume_wk,&fts_data->touch_resume_work);
		}
        else if (*blank == MSM_DRM_BLANK_POWERDOWN){
            if(id == 0){
				...
				FTS_INFO("receives EARLY_BLANK:POWERDOWN from crtx(%d), run suspend", id);
				queue_work(fts_data->tp_suspend_resume_wk,&fts_data->touch_suspend_work);
			} else{
				FTS_INFO("receives EARLY_BLANK:POWERDOWN from crtx(%d), don't run suspend", id);
			}
		}
    }
    return 0;
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(strncmp(dsi_panel_id, TOUCH_PANEL_ID, sizeof(TOUCH_PANEL_ID)-1) != 0){
		FTS_ERROR("panel not connected");
		return -ENODEV;
	}
...
//<ASUS-BSP> add touch test node ++++++
    create_focal_touch_proc_file();
//<ASUS-BSP> add touch test node -----

//ASUS_BSP add touch resume/suspend workqueue +++
	ts_data->tp_suspend_resume_wk = create_singlethread_workqueue("fts_touch_wq");
	if(!ts_data->tp_suspend_resume_wk){
		FTS_ERROR("touch resume_suspend wk create fail");
		goto exit_create_singlethread;
	}
	INIT_WORK(&ts_data->touch_suspend_work, fts_suspend_work);
	INIT_WORK(&ts_data->touch_resume_work, fts_resume_work);
//ASUS_BSP add touch resume/suspend workqueue ---
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    //ASUS_BSP  for factory test, enable touch when touch resume +++
    if (FOCAL_TOUCH_DISABLE){
		FOCAL_TOUCH_DISABLE = false;
	}
	//ASUS_BSP  for factory test, enable touch when touch resume ---
...
#if FTS_POWER_SOURCE_CUST_EN
	//if (fts_gesture_mode_en == DISABLE) {
