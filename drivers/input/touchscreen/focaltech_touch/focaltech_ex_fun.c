...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (fts_i2c_read_reg(client, FTS_REG_FW_VER, &fwver) < 0) {
        num_read_chars = snprintf(buf, PAGE_SIZE, "I2c transfer error in read fw version!\n");
    }
    if (fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &vdid) < 0) {
		num_read_chars = snprintf(buf, PAGE_SIZE, "I2c transfer error in read vendor id!");
	}
#if FTS_ESDCHECK_EN
    fts_esdcheck_proc_busy(0);
#endif
