...
#define PROC_CTRL2_LEDS "driver/ctrl2_leds"
...
static uint8_t g_flash_index[MAX_LED] = {1,2,3};
static uint8_t g_flash_id[MAX_CTRL] = {0,1};
...
static int get_flash_state(uint32_t id)
{
...
static int asus_led_release(uint32_t id)
{
	...
	if(!g_camera_in_use)
		fctrl->flash_state = CAM_FLASH_STATE_INIT;//set state to init, to avoid state mismatch in msm use
	mutex_unlock(&(fctrl->flash_mutex));
	return rc;
}
...
static ssize_t flash_led_store(struct file *dev, const char *buf, size_t count, loff_t *loff)
{
	...
	pr_info("node %d, command %d %d, rc %d, ATD status %d, camera_in_use %d, flash state %d\n",*index, mode, on, rc, g_ATD_status,g_camera_in_use,get_flash_state(id));
...
static ssize_t flash_leds_store(struct file *dev, const char *buf, size_t count, loff_t *loff)
{
	...
	ssize_t ret_len = count;
	uint8_t* id_data = (uint8_t*)PDE_DATA(file_inode(dev));
	uint32_t id;
	...
	{
		mA[1] = 0;
		pr_info("DRACO only have one LED, disable second LED\n");
	}
#endif
	...
	{
		mA[1] = 0;
		pr_info("Flash ID 1 only have one LED, disable current set for LED2\n");
	}
...
static void create_flash_proc_files(uint32_t id)
{
	if(id == 0)
	{
		...
		create_proc_file(PROC_CTRL1_LEDS, &flash_leds_proc_fops, &g_flash_id[0]);
	}
	else if(id == 1)
	{
		create_proc_file(PROC_ATD_FLASH3, &flash_led_proc_fops, &g_flash_index[2]);
		create_proc_file(PROC_CTRL2_LEDS, &flash_leds_proc_fops, &g_flash_id[1]);
	}
	else
