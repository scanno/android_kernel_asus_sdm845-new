...
static void set_dit_afc_data(void)
{
	...
	if(read_uint32_from_text_file(DAC_10CM_DIT_K,&dac_10cm) == 0)
	{
		if(read_uint32_from_text_file(DAC_50CM_DIT_K,&dac_50cm) == 0)
		{
...
{
	int num;
	num=read_file_into_buffer(dit_eeprom_file_name[camera_id],e_ctrl->cal_data.mapdata,8192);
	if(num>0)
	{
		pr_info("[DIT_EEPROM] read %d bytes from %s",num,dit_eeprom_file_name[camera_id]);
	}else if((num = read_file_into_buffer(dit_eeprom_golden_file_name[camera_id],e_ctrl->cal_data.mapdata,8192)) >0)
	{
...
void asus_cam_sensor_init(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if(!g_sensor_init_state[s_ctrl->id])
	{
		...
		if(s_ctrl->id == CAMERA_0)//check id??
		{
			ois_probe_check();//trigger ois check here
