...
static ssize_t ois_cali_proc_write(struct file *dev, const char *buf, size_t count, loff_t *ppos)
{
	...
	rc = sysfs_write_dword_seq_change_line(OIS_GYRO_K_OUTPUT_FILE_NEW,gyro_data,GYRO_K_REG_COUNT,2,1);
...
void ois_probe_check(void)
{
	...
	if(ois_power_up(ois_ctrl) != 0)
	{
		pr_err("ois power up failed\n");
		return;
	}
	F40_WaitProcess(ois_ctrl,0,__func__);//waiting after power on
