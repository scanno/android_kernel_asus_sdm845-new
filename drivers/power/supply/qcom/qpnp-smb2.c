...
extern struct wake_lock adc_check_lock;
extern struct wake_lock ChargerModeLock;
extern struct wake_lock UsbCable_Lock;
...
static int smb2_remove(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, NULL);
	wake_lock_destroy(&adc_check_lock);
	wake_lock_destroy(&ChargerModeLock);
	wake_lock_destroy(&UsbCable_Lock);
