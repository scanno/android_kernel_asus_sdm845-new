...
struct wake_lock adc_check_lock;
struct wake_lock ChargerModeLock;
...
//ASUS_BSP +++
#define CHGLimit_PATH "/persist/charger/CHGLimit_kernel"
...
static int asus_print_all(void)
{
    ...
    if((val&BIT(4)?1:0)||(val&BIT(5)?1:0))
    {
        ASUSEvtlog("INOV Triggered pmi_THERM_STS[4582]=0x%x\n",val);
    }
...
{
    struct proc_dir_entry *charger_inov_enable = proc_create(charger_inov_enable_PROC_FILE, 0444, NULL, &charger_inov_enable_fops);
...
//	printk("[BAT][alarm]Jeita_Alarm triggered\n");
	...
	if (asus_get_prop_usb_present(&chip_dev->chg) == true) {
		wake_lock_timeout(&UsbCable_Lock, 2 * HZ);
		cancel_delayed_work(&chip_dev->chg.asus_chg->asus_batt_temp_work);
		schedule_delayed_work(&chip_dev->chg.asus_chg->asus_batt_temp_work, 0);
	}
...
static int asus_do_soft_jeita(struct smb_charger *chg)
{
    ...
    if (aicl_done) {
        ASUSEvtlog("aicl done, aicl_result=%d\n", aicl_result);
    }
    if (aicl_fail) {
    ...
    // re-run aicl when aicl done && aicl result lower than sw icl config
    if ((ret == 0) && aicl_done && (aicl_result < ((int)icl_max * 25 * 4 / 5))) {
        CHG_DBG("%s: re-run aicl\n", __func__);
    ...
    if (aicl_fail || usb_100_wa) {
        // Rerun APSD
        smblib_read(chg, USBIN_SOURCE_CHANGE_INTRPT_ENB_REG, &reg);
        CHG_DBG("JEITA re-running APSD after aicl fail or usb 100 wa, pmi[0x1369]=%02X\n",reg);
    ...
    switch (apsd_result->bit) {
        ...
        case FLOAT_CHARGER_BIT:
            if ((3 == asus_chg->ufp_mode) && !asus_chg->legacy_cable_flag) {
                ...
                if (SDP_CHARGER_BIT == apsd_result->bit) {
...
#define	TYPE_C_3A_ICL_THD		0x64 // 2500mA / 25mA = 100 = 0x64

void check_legacy_for_nonstandard_ac(void)
{
    u8 stat=0, icl_ret=0;
    int icl_thd, prev_legacy;
    bool reset= false;
    struct smb_charger *chg = &chip_dev->chg;
    struct asus_charger *asus_chg = chg->asus_chg;
...
set_current:
    ...
    if (3 == asus_chg->ufp_mode && !asus_chg->legacy_cable_flag) {
		...
		if ((asus_chg->BR_countrycode_flag == COUNTRY_BR) || 
			(asus_chg->BR_countrycode_flag == COUNTRY_IN)) {
...
post_proc:
    ...
    if(wake_lock_active(&adc_check_lock)){
        wake_unlock(&adc_check_lock);
    }
}
...
void asus_handle_usb_insertion(struct work_struct *work)
{
	...
	if(g_charger_mode){
		...
		wake_lock_timeout(&ChargerModeLock, 90*HZ);
	}
    ...
    if (asp1690_ready && !asus_chg->suspend_flag) {
        ...
        case USB_THERM_ALERT_LOW_TRIGGER: // temp over 70 deg
            ...
            if (usb_present) { // usb in
                ...
                ASUSErclog(ASUS_USB_THERMAL_ALERT, "Thermal Alert is triggered");
        ...
        case USB_THERM_ALERT_HIGH_TRIGGER:// temp below 60 deg
            ...
            // release alert and report to UI only when cable not in
            if (!usb_present) {
                // usb alert = 0, clear state
...
void asus_charger_init_config(struct smb_charger *chg)
{
    ...
    chg->asus_chg->asus_tb.rchg_cfg = RCHG_4V252;
    ...
    //~ rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG,
            //~ SUSPEND_ON_COLLAPSE_USBIN_BIT, 0);
    //~ if (rc < 0) {
        //~ dev_err(chg->dev, "Couldn't clear SUSPEND_ON_COLLAPSE_USBIN_BIT rc=%d\n", rc);
    //~ }
    ...
    wake_lock_init(&adc_check_lock, WAKE_LOCK_SUSPEND, "adc_check_Lock");
	wake_lock_init(&ChargerModeLock, WAKE_LOCK_SUSPEND, "ChargerModeLock");	
