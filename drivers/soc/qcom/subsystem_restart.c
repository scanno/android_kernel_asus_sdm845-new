...
static ssize_t make_crash_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    //struct subsys_device *subsys = to_subsys(dev);
    char p[] = "echo 1 to make this subsys force reset";

    return snprintf(buf, PAGE_SIZE, "%s\n", p);
}

static ssize_t make_crash_store(struct device *dev,
        struct device_attribute *attr, const char *buf,
        size_t count)
{
    struct subsys_device *subsys = to_subsys(dev);
    ...
    //subname = subsys->desc->name;

    p = memchr(buf, '\n', count);
    if (p)
        count = p - buf;

    if (!strncasecmp(buf, "1", count)) ///bevis
    {
        printk("Bevis Triggle Modem Reset ! %s\n",subsys->desc->name);

        asus_force_reset = 1;

        if (subsys->desc->name) {
            rc = subsystem_restart(subsys->desc->name);
            if (rc < 0)
                printk("%s: Error %d restarting %s\n",__func__, rc, subsys->desc->name);
        } else {
            printk("Bevis  sub-sys name error !\n");
        }

        asus_force_reset=0; // reset to default
...
static struct device_attribute subsys_attrs[] = {
	...
	__ATTR(make_crash, 0644, make_crash_show, make_crash_store),
	__ATTR_NULL,
};
